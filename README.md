...
- **Lecturer**: creates assignments as either (a) a file/text upload brief (optionally attaching a handout file of their own), or (b) an interactive form. For interactive forms, every question is set up front, in one continuous flow, before the assignment is saved at all — there's no separate "add questions later" step. Once saved, an assignment appears as a compact row in "My Assignments"; clicking it opens its full details (and, for forms, its questions) rather than showing everything inline. Objective questions are auto-graded on submission; theory questions are graded by the lecturer afterward. Assignment marking is entirely the lecturer's own call. Course results are entered separately as a single score, converted to a letter grade using the admin-defined grade scale.
- **Student**: registers for courses, then per assignment either submits text/a file (which can be resubmitted before grading), or answers an interactive form. Interactive form answers are **one-time only** — once submitted, the assignment switches to a read-only view showing exactly what was submitted; there is no retry, resubmission, or edit. For form-type assignments, the compiled score updates live as the lecturer grades theory answers.
